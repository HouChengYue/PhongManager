...
import edu.feicui.app.phone.service.BootBroadcastReceiver;
import edu.feicui.app.phone.util.NotificationUtil;
...
public class SettingActivity extends BaseActivity {
	...
	private void initMainButton() {
		 ...
		 * 是否开机自启
		 */
		tb_boot=(ToggleButton)findViewById(R.id.tb_toggle_reboot);
		tb_boot.setChecked(BootBroadcastReceiver.isOpenboot(getApplicationContext()));
		tb_boot.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked){
					BootBroadcastReceiver.On();
				}else{
					BootBroadcastReceiver.Off();
				}
			}
		});
	}
	...
	protected void onDestroy() {
		...
		BootBroadcastReceiver.setOpenboot(getApplicationContext(),tb_boot.isChecked());
	}
