...
public class AppInfoManager {
	...
	public static final int RUNING_APP_TYPE_All= 2;
	...
	public Map<Integer, List<RuningAppInfo>> getRuningAppInfos() {
		...
		List<RuningAppInfo> allapp = new ArrayList<RuningAppInfo>();
		// 获取所有正在运行应用
		List<RunningAppProcessInfo> appProcessInfos = activityManager.getRunningAppProcesses();
		for (RunningAppProcessInfo appProcessInfo : appProcessInfos) {
			...
			if (importance >= RunningAppProcessInfo.IMPORTANCE_SERVICE) {
				...
				try {
					...
					ApplicationInfo applicationInfo = packageManager.getApplicationInfo(packageName,
							PackageManager.GET_META_DATA | PackageManager.GET_SHARED_LIBRARY_FILES | PackageManager.GET_UNINSTALLED_PACKAGES);
		...
		runingAppInfos.put(RUNING_APP_TYPE_All, allapp);
